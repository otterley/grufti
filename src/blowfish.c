 ...
 *
 * blowfish.c - encryption and decryption, public domain.
 *
...
void debug_blowfish()
{
	...
	debug("%d of %d boxes in use: \n",tot,BOXES);

	for(i=0; i<BOXES; i++)
		if (box[i].P != NULL)
			debug("Box %d: (age: %d)",i+1,time(NULL)-box[i].lastuse);
...
void blowfish_init(UBYTE_08bits *key, char keybytes)
{
	...
	/* uh... this is over 4k */
	box[bx].P = (UWORD_32bits *)xmalloc((bf_N+2)*sizeof(UWORD_32bits));
	box[bx].S = (UWORD_32bits **)xmalloc(4*sizeof(UWORD_32bits *));
	for(i=0; i<4; i++)
		box[bx].S[i] = (UWORD_32bits *)xmalloc(256*sizeof(UWORD_32bits));
	bf_P = box[bx].P;
	bf_S = box[bx].S;
...
void encrypt_pass(char *text, char *new)
{
	...
	blowfish_init(text,(char)strlen(text));
	left = SALT1;
	right = SALT2;
...
char *encrypt_string(char *key, char *str)
{
	...
	dest = (char *)xmalloc((strlen(str)+9)*2);

	/* pad fake string with 8 bytes to make sure there's enough */
	s = (char *)xmalloc(strlen(str)+9);
	strcpy(s,str);
	...
	for(i=0; i<8; i++)
		*p++=0;
	blowfish_init(key,(char)strlen(key));
	p = s;
	d = dest;
	while(*p) 
	{
...
char *decrypt_string(char *key, char *str)
{
	...
	dest = (char *)xmalloc(strlen(str)+12);

	/* pad encoded string with 0 bits in case it's bogus */
	s = (char *)xmalloc(strlen(str)+12);
	strcpy(s,str);
...
int main()
{
	char shouldbe[] = "L1CND2gNrsE0nLW4C0YQzLN2";
	char cipher_shouldbe[] = "+pVxuu2UYwKP1";
	char pass[] = "MyPassword";
	char cipher[80];
	char *p1, *p2;
	int ok1 = 0, ok2 = 0, ok3 = 0;

	printf("Testing module: blowfish\n");
	p1 = encrypt_string("mykey", pass);
	if(strcmp(p1, shouldbe) == 0)
	{
		printf("  encyption: passed\n");
		ok1 = 1;
	}
	else
		printf("  encyption: failed\n");

	p2 = decrypt_string("mykey", p1);
	if(strcmp(pass, p2) == 0)
	{
	...
	encrypt_pass(pass, cipher);
	if(strcmp(cipher_shouldbe, cipher) == 0)
	{
		printf("  password cipher: passed\n");
		ok3 = 1;
	}
	else
		printf("  password cipher: failed\n");


	if(ok1 && ok2 && ok3)
		printf("blowfish: passed\n");
	else
		printf("blowfish: failed\n");
